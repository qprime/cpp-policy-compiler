...
Detect what exists, then run exactly what's configured. Run the project's tests, linter, and type checker — whatever the project ships. Check `pyproject.toml`, `package.json`, a `Makefile`, or CI config to learn the exact commands.
...
If a check fails on code this session did not touch, stop and report it to the user with the output. Do not fix it and do not commit around it.
...
When the implementation closes an issue, post the Implementation Summary as a comment on that issue.
