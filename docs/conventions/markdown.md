...
Rules for AI agents writing markdown files in projects created from this baseline. They govern the content of those files. They do not govern replies in conversation, which follow the active output style.

Markdown here is a content-bearing artifact. Humans read these notes, specs, READMEs, and CLAUDE files, and other agents parse them. The structure has to behave as a structure.

Each rule has: the rule (imperative), a right/wrong example, and the failure mode if violated.
...
```markdown
# Right
The loader reads `config/settings.yaml` and writes to
`output_dir`. Run `make build` to apply.
...
```markdown
# Right
See [the invariants](../INVARIANTS.md) and [GL-2](../INVARIANTS.md#gl-2).

# Wrong
See https://github.com/owner/project/blob/main/INVARIANTS.md
```
...
Frontmatter is for fields a tool consumes (`status:`, `id:`, `argument-hint:`). Don't add frontmatter that no code or downstream agent reads — that's documentation pretending to be data.

```markdown
# Right (consumed by a tool)
---
status: draft
id: KB-4
---
...
described in `notes/patterns.md:280`.
